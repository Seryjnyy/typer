...
export default function PlaylistPage() {
    ...
    const randomSong = useMemo(() => {
        const randomSong = playlistSongs.length > 0 ? playlistSongs[Math.floor(Math.random() * playlistSongs.length)] : null
            ...
            <ScrollArea className={`h-[100%]  pb-2  flex flex-col relative `}>
                ...
                >
                    ...
                    <div className="space-y-4 w-full">
                        <div>
                            <PlaylistHeader>
                                <PlaylistBanner playButtonHover playlist={playlist} size={"xl"} />
                                <div className="flex flex-col justify-center items-start px-8">
                                    <span className="text-foreground/80 select-none">Playlist</span>
                        ...
                        <div className="flex items-end justify-between   w-full ">
                            <div className="flex gap-4 border border-dashed p-2 rounded-lg w-fit ">
                                <span className="text-xs text-muted-foreground">{playlistSongs.length} songs</span>
                            </div>

                            <div className="flex gap-2   items-center  justify-between">
                                <div className="w-fit">
                                    <Link to={`/songs/playlist/${playlist.id}/edit`}>
                                ...
                                <PlaylistPopover playlist={playlist} exclude={{ viewMore: true }} />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="pt-12 px-[1.75rem] sm:px-[4.25rem]">
                    <h2 className="text-3xl font-bold text-muted-foreground pb-8 select-none">Songs</h2>
                    {playlistSongs.length > 0 && (
                        <SongsList
                            exclude={{ deleteButton: true }}
                            songsList={playlistSongs.map((songID) => getSongData(songID)).filter((x) => x != undefined)}
                ...
                <div className="w-full  flex items-center pt-24 flex-wrap gap-2 sm:gap-12 px-[1.75rem] sm:px-[4.25rem]">
                    <div className="flex  text-muted-foreground gap-1">
                        ...
                        <span className="text-xs">{formatTimestamp(playlist.createdAt)}</span>
                    </div>
                    <div className="flex  text-muted-foreground gap-1">
                        <span className="text-xs opacity-70">last modified at:</span>
                        <span className="text-xs">{formatTimestamp(playlist.lastModifiedAt)}</span>
                    </div>
                </div>
            </ScrollArea>
        </div>
