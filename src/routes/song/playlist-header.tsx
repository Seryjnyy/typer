...
interface PlaylistBannerProps extends React.ButtonHTMLAttributes<HTMLDivElement>, VariantProps<typeof playlistBannerVariants> {
    playlist: Playlist
    className?: string
    playButton?: boolean
...
const PlaylistBanner = ({ playlist, className, size, playButton = false, playButtonHover = false, ...props }: PlaylistBannerProps) => {
    ...
    const renderBanner = () => {
        switch (playlistSongs.length) {
            ...
            case 2: {
                const [s1, s2] = playlistSongs.map((id) => songsList.find((x) => x.id == id))
                return (
                    <div className="w-full h-full">
                        <div className="grid grid-cols-2 w-full h-full">
                            <div className="w-full h-full" style={s1 && coverAsStyle(parseGeneratedCoverString(s1.cover))}></div>
                            <div className="w-full h-full" style={s2 && coverAsStyle(parseGeneratedCoverString(s2.cover))}></div>
                        </div>
                    </div>
                )
            }
            case 3: {
                const [s1, s2, s3] = playlistSongs.map((id) => songsList.find((x) => x.id == id))
                return (
            ...
            default: {
                const [s1, s2, s3, s4] = playlistSongs.slice(0, 4).map((id) => songsList.find((x) => x.id == id))
                return (
    ...
    return (
        <div
            className={cn(playlistBannerVariants({ size: size }), "relative", "rounded-[10px] overflow-hidden group ", className)}
            {...props}
        >
            {/* {element} */}
            {renderBanner()}
            {(playButtonHover || playButton) && (
                <div
                    className={cn("z-40 peer absolute top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%]", {
...
const playlistDetailVariants = cva("text-ellipsis overflow-hidden whitespace-nowrap", {
    variants: {
        length: {
...
interface PlaylistDetailProps extends React.ButtonHTMLAttributes<HTMLDivElement>, VariantProps<typeof playlistDetailVariants> {
    playlist: Playlist
    className?: string
}

const PlaylistDetail = ({ playlist, className, length, ...props }: PlaylistDetailProps) => {
    return (
        <div className={cn("flex flex-col leading-tight justify-center", className)} {...props}>
            <span className={cn("text-muted-foreground text-xs", playlistDetailVariants({ length }))}>Playlist</span>
            <span className={cn(playlistDetailVariants({ length }), "pb-2 pl-1")}>{playlist.title}</span>
        </div>
    )
}

const PlaylistHeader = ({ className, children }: { className?: string; children?: ReactNode }) => {
    return (
