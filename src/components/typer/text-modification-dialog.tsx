...
import { textModification } from "@/lib/utils.ts"
...
const CurrentView = () => {
    ...
    return (
        <div className="w-full border rounded-md p-2 flex items-center flex-col">
            <span className="text-sm font-bold pb-1 text-muted-foreground">Your current view</span>
            <div className="border rounded p-2 w-full">
                <Ch variant={"correct"}>{before}</Ch>
                <Ch variant={currentChVariant}>{current}</Ch> <Ch variant={afterChVariant}>{after}</Ch>
            </div>
        </div>
    )
}
...
export default function TextModificationDialog() {
    ...
    const isTxtModificationChanged = txtMod.letterCase != "normal" || txtMod.punctuation != "normal" || txtMod.numbers != "normal"

    const harderOptions = useTextModificationsStore.use.harderOptions()
    const setHarderOptions = useTextModificationsStore.use.setHarderOptions()
    const resetHarderOptions = useTextModificationsStore.use.resetHarderOptions()
    const resetTextModifications = useTextModificationsStore.use.resetTextModifications()
        ...
        <Dialog>
                    ...
                    <TooltipTrigger asChild>
                        <DialogTrigger className="group p-2">
                            <div className="relative ">
                                ...
                                {isTxtModificationChanged && <span className="text-emerald-400 absolute bottom-0 left-2 font-bold">*</span>}

                                {isHarderOptionsChanged && <span className="text-red-600 absolute bottom-0 right-2 font-bold">*</span>}
                            </div>
                        </DialogTrigger>
                    </TooltipTrigger>
                    <TooltipContent>
            ...
            <DialogContent className="h-[72vh] flex flex-col">
                <DialogHeader className="relative">
                    <Button className="absolute -top-[1.17rem] right-4" size={"icon"} variant={"ghost"} onClick={onResetAll}>
                        <ReloadIcon className="w-3 h-3" />
                    </Button>
                    <DialogTitle className="sr-only">Text modification</DialogTitle>
                    <DialogDescription className="sr-only">
                        Make it easier by changing the text or make it harder by changing how things are displayed.
                    </DialogDescription>
                </DialogHeader>
                <Tabs defaultValue="easier" className="pt-4">
                    <TabsList className="w-full mb-3">
                        <TabsTrigger value="easier" className="w-full">
                            Easier
                            {isTxtModificationChanged && <span className="text-emerald-400  font-bold">*</span>}
                        </TabsTrigger>
                        <TabsTrigger value="harder" className="w-full">
                            Harder
                            {isHarderOptionsChanged && <span className="text-red-600 font-bold">*</span>}
                        </TabsTrigger>
                    </TabsList>
                        ...
                        <div className="p-2 space-y-2">
                            <div className="border rounded-sm p-2 space-y-2">
                                <span className="text-xl font-bold">
                                    Letter case
                                    {txtMod.letterCase != "normal" ? <span className="text-primary">*</span> : ""}{" "}
                                    <span className="text-xs text-muted-foreground">{"(a-z A-Z)"}</span>
                                </span>
                                <ToggleGroup
                                    className="flex justify-start items-center "
                                    type="single"
                                    value={txtMod.letterCase}
                                    onValueChange={(val) => onChangeLetterCase(val)}
                                >
                                    <ToggleGroupItem value="normal" aria-label="Toggle bold">
                                        Normal
                                    </ToggleGroupItem>
                                    <ToggleGroupItem value="upper" aria-label="Toggle italic">
                                        UPPER
                                    </ToggleGroupItem>
                                    <ToggleGroupItem value="lower" aria-label="Toggle underline">
                                        lower
                                    </ToggleGroupItem>
                                </ToggleGroup>
                            </div>
                            <div className="border rounded-sm p-2 space-y-2">
                                <span className="text-xl font-bold">
                                    Punctuation
                                    {txtMod.punctuation != "normal" ? <span className="text-primary">*</span> : ""}{" "}
                                ...
                                <ToggleGroup
                                    ...
                                    onValueChange={(val) => onChangePunctuation(val)}
                                >
                                    <ToggleGroupItem value="normal" aria-label="Toggle bold">
                                        Normal
                                    </ToggleGroupItem>
                                    <ToggleGroupItem value="removed" aria-label="Toggle italic">
                                        Removed
                                    </ToggleGroupItem>
                                </ToggleGroup>
                            </div>
                            <div className="border rounded-sm p-2 space-y-2">
                                <span className="text-xl font-bold">
                                    Numbers
                                    {txtMod.numbers != "normal" ? <span className="text-primary">*</span> : ""}{" "}
                                ...
                                <ToggleGroup
                                    ...
                                    onValueChange={(val) => onChangeNumbers(val)}
                                >
                                    <ToggleGroupItem value="normal" aria-label="Toggle bold">
                                        Normal
                                    </ToggleGroupItem>
                            ...
                            <div>
                                ...
                                <div className="border rounded-sm p-2 space-y-2">
                                    <span className="text-xl font-bold">
                                        ...
                                        <span className="text-xs text-muted-foreground">{"(Pick and choose.)"}</span>
                                    </span>
                                        ...
                                        <div>
                                            <div className="flex flex-col justify-center items-center rounded-sm border p-2 w-fit space-y-1">
                                                <div className="border rounded p-2 w-full">
                                                    ...
                                                    <Ch variant={"current"}>e</Ch> ____ __
                                                </div>
                                                <Toggle
                                                    pressed={harderOptions.cantSeeAhead}
                                                    onPressedChange={(val) => onChangeCantSeeAhead(val)}
                                                >
                                                    Can't see ahead
                                                </Toggle>
                                            </div>
                                        ...
                                        <div className="space-y-2 w-fit">
                                            <div>
                                                <div className="flex flex-col justify-center items-center rounded-sm border p-2 space-y-1 w-full">
                                                    <div className="border rounded p-2 w-full">
                                                        <Ch variant={"correct"}>Som</Ch>
                                                        <Ch variant={"currentInvisible"}>e</Ch> ____ __
                                                    </div>
                                                    <Toggle
                                                        pressed={harderOptions.cantSeeCurrent}
                                                        onPressedChange={(val) => onChangeCantSeeCurrent(val)}
                                                        disabled={!harderOptions.cantSeeAhead}
                                                    >
                                                        Can't see current
                                                    </Toggle>
                                                </div>
                                            ...
                                            <div>
                                                <div className="flex flex-col justify-center items-center rounded-sm border p-2 w-full space-y-1">
                                                    <div className="border rounded p-2 w-full">
                                                        <Ch variant={"correct"}>Som</Ch>
                                                        <Ch variant={"current"}>e</Ch>{" "}
                                                    </div>
                                                    <Toggle
                                                        pressed={harderOptions.cantSeeUnderlines}
                                                        onPressedChange={(val) => onChangeCantSeeUnderlines(val)}
                                                        disabled={!harderOptions.cantSeeAhead}
                                                    >
