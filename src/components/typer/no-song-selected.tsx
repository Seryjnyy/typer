import SongCarousel from "@/components/song-carousel.tsx"
import { Button } from "@/components/ui/button.tsx"
import usePlaySong from "@/lib/hooks/use-play-song.tsx"
import usePlaylists from "@/lib/hooks/use-playlists.tsx"
import { usePlaylistStore } from "@/lib/store/playlist-store.tsx"
import { useSongStore } from "@/lib/store/song-store.tsx"
import { useUiStateStore } from "@/lib/store/ui-state-store.tsx"
import { Song } from "@/lib/types.ts"
import { cn, shuffleArray, splitSongIntoVerses } from "@/lib/utils.ts"
...
export default function NoSongSelected() {
    ...
    const handlePlayRandomPlaylist = () => {
        if (playlists.length > 0) {
            const rand = playlists[Math.floor(Math.random() * playlists.length)]
    ...
    const handlePlayRandomVerse = () => {
        if (shuffled.length > 0) {
            ...
            const randomVerse = (verses.length > 0 ? shuffleArray(verses)[0] : "") as string
    ...
    return (
        <div className={cn("flex w-full items-center justify-center  flex-col ")}>
            <h2 className="font-bold text-3xl">No song selected.</h2>
            <div className="flex flex-col justify-center items-center  ">
                {songs.length > 0 && (
                    <div className="space-y-1">
                        <h3 className=" text-muted-foreground mx-auto w-fit text-md mb-8">Here are some of your songs.</h3>

                        <div
                            className={cn(
                                "overflow-hidden w-[100vw] ",
                                isQueueWindowOpen ? "sm:w-[calc(100vw-16.5rem)]" : "sm:w-[calc(100vw-1.7rem)]"
                            )}
                        >
                ...
                <div className="flex flex-col gap-3">
                    {songs.length == 0 && (
                        <div className="flex flex-col justify-center items-center">
                            <span className="text-sm text-muted-foreground">Seems like you don't have any songs yet :/</span>
                            <span className="text-sm text-muted-foreground">No worries you can add some.</span>
                        </div>
                    )}
                    <Link to={"/songs/?tab=add-song"}>
                        <Button className="space-x-2 w-full" variant={"secondary"}>
                            <PlusIcon />
                            <span>Add new song</span>
                        </Button>
                    </Link>
                    <div className="pt-12 gap-2 flex flex-wrap justify-center">
                        <Button className="space-x-2" variant={"outline"} disabled={songs.length == 0} onClick={handlePlayRandomSong}>
                            <PlayIcon />
                            <span>Play random song</span>
                        </Button>
                        <Button className="space-x-2" variant={"outline"} disabled={songs.length == 0} onClick={handlePlayRandomVerse}>
                            <PlayIcon />
