...
const useQueue = () => {
    ...
    const queue = useMemo(() => {
        const res = queueSongsList.map((songId) => songList.find((x) => x.id == songId))
...
export const MobileQueue = () => {
    ...
    const { queue, currentID, currentSong, clearQueue, enqueueRandomSongs, removeFromQueue, totalAvailableSongs } = useQueue()
    ...
    return (
        <div className="h-full rounded-t-sm    w-full overflow-hidden">
            <ScrollArea className=" h-[calc(100%)]  pl-1 pr-3 pb-1 border-t" style={isQueueColoured ? coverAsBgGradientStyle : undefined}>
                {queue.map((song, index) => (
                    ...
                    >
                        <div className={"flex items-center " + (currentID == song.id ? "text-foreground" : "text-muted")}>
                            <span className="">{index + 1}</span>
                        </div>
                        <SongHeader className="pl-3">
                            <SongBanner song={song} playButton />
                            <SongDetail song={song} isCurrent={song.id == currentID} />
                        </SongHeader>
                        <div className="flex items-center">
                            <div className="flex justify-between">
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <Button onClick={() => removeFromQueue(song.id)} size={"icon"} variant={"ghost"}>
                                            <Cross1Icon />
                                        </Button>
                                    </TooltipTrigger>
                ...
                {queue.length > 0 && (
                    <Button className="w-full mt-1 text-xs text-muted-foreground" variant={"ghost"} size={"sm"} onClick={clearQueue}>
                        Clear queue
                    </Button>
                )}
            </ScrollArea>
...
export default function Queue() {
    ...
    const { queue, currentID, currentSong, clearQueue, enqueueRandomSongs, removeFromQueue, setQueue, totalAvailableSongs } = useQueue()
    ...
    return (
        <div
            className={cn("h-full border rounded-md py-4  w-[15rem] overflow-hidden")}
                                ...
                                <Draggable draggableId={song.id} index={index} key={song.id}>
                                    {(provided) => (
                                        <div
                                            ...
                                            className={cn(
                                                "border p-3 rounded-md space-y-2 group mt-2 bg-transparent flex ",
                                                isQueueColoured
                                                    ? currentID == song.id
                                                        ? "backdrop-brightness-50"
                                                        : "backdrop-brightness-75"
                                        ...
                                        >
                                            <div
                                                className={"flex items-center " + (currentID == song.id ? "text-foreground" : "text-muted")}
                                            >
                                                <span className="">{index + 1}</span>
                                            </div>
                                            <SongHeader className="pl-3">
