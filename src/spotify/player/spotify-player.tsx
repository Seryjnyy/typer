...
import { usePlaybackState, useWebPlaybackSDKReady, WebPlaybackSDK } from "react-spotify-web-playback-sdk"
...
const Testing = () => {
    ...
    const { togglePlayURI } = usePlaySongURI(playableSong?.spotifyUri ?? "")
...
const useCurrentTrackAndCurrentSongChecker = () => {
    const playbackState = usePlaybackState()
    const { currentPlayableSong, setPlayableSong } = usePlaySongThroughSpotify()
...
const WithAccessToken = ({ render }: { render: (accessToken: string) => ReactNode }) => {
