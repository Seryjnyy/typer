...
type Actions = {
    ...
    enqueue: (songId: string, setNewCurrent?: boolean, position?: number) => void
...
if (preferenceStore) {
    const storedQueueStoragePreference = JSON.parse(preferenceStore).state?.queueStorage
    ...
    persist(
        (set, get) => ({
            ...
            next: () =>
                set(() => {
                    const state = get()
                    const currentId = state.songs.findIndex((x) => x == state.current)
            ...
            setCurrent: (newCurrentSong) => set(() => ({ current: newCurrentSong })),
            setSongs: (songs) =>
                set(() => {
                    let current = null
            ...
            enqueue: (songId, setNewCurrent, position) =>
                set(() => {
                    ...
                    // Insert in position
                    // Or just enqueue
                    if (position && position >= 0 && position < state.songs.length) {
                        ...
                        //  I guess this is enqueuing based on a position
            ...
            getNextSong: () => {
                ...
                const currentIndex = state.songs.findIndex((songID) => songID == state.current)
        ...
        }),
        {
            ...
            storage:
                preferredStorageType == "localStorage" ? createJSONStorage(() => localStorage) : createJSONStorage(() => sessionStorage),
