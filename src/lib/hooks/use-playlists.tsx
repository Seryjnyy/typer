...
export default function usePlaylists() {
    ...
    // TODO : playFirstSong is not used
    const playPlaylist = (playlistID: string, enqueuePlaylist?: boolean, playFirstSong?: boolean) => {
    ...
    const removeFromPlaylist = (playlistID: string, songID: string) => {
        ...
        editPlaylist({
            ...playlist,
            dontUsesDirectlySongs: [...playlistSongs.filter((x) => x != songID)],
        })
    }
    ...
    const getPlaylistSongsWithData = (playlistID: string) => {
        ...
        return validPlaylistSongs.map((songID) => songsList.find((song) => song.id == songID)).filter((song) => song != undefined)
    }

    const createPlaylist = ({ title, songs }: { title: string; songs?: string[] }) => {
